...
class Projects(BaseModel):
    ...
    def clean(self):
        ...
        if self.status not in valid_statuses:
            raise ValidationError(f"Invalid status: '{self.status}'. Valid choices are: {valid_statuses}")

        # Validate title uniqueness
        if Projects.objects.filter(title=self.title).exclude(id=self.id).exists():
            raise ValidationError(f"A project with the title '{self.title}' already exists.")

        # Validate description similarity
        existing_descs = Projects.objects.filter(created_at__year=now().year).exclude(id=self.id).values_list('desc', flat=True)
        if existing_descs:
            similar_descs = get_similarities(self.desc, list(existing_descs))
            if similar_descs:
                raise ValidationError("The description is too similar to existing projects.")
